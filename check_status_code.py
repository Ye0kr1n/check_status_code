...
def check_res_code(url):
    ...
    url_ans = ""
    try:
        res = str(requests.get(url, timeout=0.5).status_code)  # http
        url_ans = url
        try:
            title = get_title(url)
        except:
            title = "None"
            res = str(requests.get(urls, timeout=0.5).status_code)  # https
            url_ans = urls
        try:
            title = get_title(url)
        except:
...
if __name__ == '__main__':
    ...
    for i in range(0, len(url_list), 1):
        ...
        print("[*][%s/%s]%s   status code:%s    title:%s" % (
        i + 1, total, check_res_code(url_list[i])[0], check_res_code(url_list[i])[1], check_res_code(url_list[i])[2]))
